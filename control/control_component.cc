...
bool ControlComponent::Init() {
  ...
  if (!FLAGS_use_control_submodules) {
    control_cmd_writer_ =
        ...
        // control_cmd_writer_ = node_->CreateWriter<ControlCommand>("/apollo/control_prepare");
...
bool ControlComponent::Proc() {
  ...
  double start_time_proc = millisec_since_epoch_start;
  ...
  auto millisec_since_epoch_end = duration_cast<milliseconds>(system_clock::now().time_since_epoch()).count();
  double end_time_proc = millisec_since_epoch_end;
  control_command.set_start_time(start_time_proc);
  control_command.set_end_time(end_time_proc);
